...
public:
    explicit ScreenShotSourceBasic1(QFuture<QImage> future);
...
ScreenShotSourceBasic1::ScreenShotSourceBasic1(QFuture<QImage> future)
    : m_future(future)
{
...
void ScreenShotDBusInterface1::updateSource(ScreenShotSink1 *sink,
                                            ScreenShotSource1 *source)
{
    m_sink.reset(sink);
...
{
    updateSource(sink, new ScreenShotSourceScreen1(m_effect, screen, flags));
}
                                              ...
                                              ScreenShotFlags flags, ScreenShotSink1 *sink)
{
    ...
    updateSource(sink, new ScreenShotSourceMulti1(sources));
}

void ScreenShotDBusInterface1::takeScreenShot(const QRect &area, ScreenShotFlags flags,
                                              ScreenShotSink1 *sink)
{
    updateSource(sink, new ScreenShotSourceArea1(m_effect, area, flags));
}

void ScreenShotDBusInterface1::takeScreenShot(EffectWindow *window, ScreenShotFlags flags,
                                              ScreenShotSink1 *sink)
{
    updateSource(sink, new ScreenShotSourceWindow1(m_effect, window, flags));
}
